...
## 필수 요구 사항
 ...
 ## 선택 요구 사항
    [X] API 인증을 위해 JWT(Json Web Token)를 이용해서 Token 기반 API 인증 기능을 개발하고 각 API 호출 시에 HTTP Header 에 발급받은 토큰을 가지고 호출
 ...
 ## 문제 해결 전략
    ...
    13. API 인증을 위해 JWT(Json Web Token)를 이용해서 Token 기반 API 인증 기능
        ...
        - 중복 가입을 방지
            - controller단 validation과 Entity unique annotation설정 
       
            
       
## 빌드 및 실행 방법
    1. gradle build && java -jar build/libs/project-0.0.1-SNAPSHOT.jar
    2. 데이터 파일을 업로드하여 기초 데이터를 저장한다. "데이터 파일에서 각 레코드를 데이터베이스에 저장하는 API" 
        1) /v1/file/upload
    3. 기초 데이터를 성공적으로 업로드 했다면 API 명세서의 API를 자유롭게 요청을 하면 된다.
...
---|---|---|---
/v1/file/upload|POST|200, {"fileType": "text/csv","fileRecordCount": 154,"fileByteSize": 8822}| 이터 파일에서 각 레코드를 데이터베이스에 저장
/v1/bank/list|GET  |200, {"name": "주택금융 공급 금융기관(은행) 목록","data": [{"institute_name": "국민은행","institute_code": "bnk1111"},...]}| 주택금융 공급 금융기관(은행) 목록을 출력
/v1/bank/supply-amount/top |GET |200, {"year":"2016","bank":"기타은행"}|  각 년도 별 각기관의 전체 지원 금액 중에서 가장 큰 금액의 기관명을 출력
/v1/bank/supply-amount/total |GET {period:'year'} |200, {"name" : "주택금융 공급현황",[ {"year" : "2005","total_amount" : 46201,….}},….]}| 년도별 각 금융기관의 지원금액 합계를 출력
/v1/bank/supply-amount/average |GET{bankName:'KEB_BANK'} |200, {"bank":"외환은행","support_amount":[{"year":"2008","amount":78.0},{"year":"2015","amount":1702.0}]} | 전체 년도(2005~2016)에서 외환은행의 지원금액 평균 중에서 가장 작은 금액과 큰 금액을 출력
